...
<div class="max-w-4xl mx-auto px-6 py-8">
    ...
    <div class="mb-10">
        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Amend Holiday Designation</h1>
        <p class="text-slate-500 text-sm mt-1 flex items-center gap-2">
            <i class="bi bi-pen text-emerald-600"></i>
            Adjust existing holiday parameters based on new administrative orders or proclamations.
        </p>
    </div>

    <form action="{{ route('holiday.update', $holiday->id) }}" method="POST" class="space-y-6" id="editHolidayForm">
        @csrf
        @method("PUT")
            ...
            <div class="p-8 md:p-10 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-2 text-left">
                        ...
                        <div class="relative">
                            ...
                            <input type="date" name="date" required value="{{ old('date', optional($holiday->date)->format('Y-m-d')) }}"
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="type" required
                            ...
                            <option value="Special Working"  {{ old('type', $holiday->type) === "Special Working" ? 'selected' : '' }}>Special Working Day</option>
                            <option value="Special Non-Working"  {{ old('type', $holiday->type) === "Special Non-Working" ? 'selected' : '' }}>Special Non-Working Day</option>
                            <option value="Local"  {{ old('type', $holiday->type) === "Local" ? 'selected' : '' }}>Local Holiday (BARMM)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="reference" placeholder="e.g., Proclamation No. 902, s. 2026" value="{{ old('reference', $holiday->reference) }}" maxlength="150"
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="remarks" rows="3" placeholder="Additional details or implementation notes..." maxlength="500"
                              class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-800 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all placeholder:text-slate-300 placeholder:font-normal">{{ old('remarks', $holiday->remarks) }}</textarea>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const holidayForm = document.getElementById('editHolidayForm');
        ...
        if (holidayForm) {
            holidayForm.addEventListener('submit', function(e) {
                ...
                Swal.fire({
                    ...
                    text: "You are about to modify this holiday's record.",
                    icon: 'warning',
