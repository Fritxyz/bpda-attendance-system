    ...
    <nav class="flex mb-8 text-sm" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-2">
            <li class="flex items-center gap-2">
                ...
                <span class="text-gray-500 hover:text-emerald-600 transition flex items-center gap-1">Employee</span>
            </li>
            <li class="flex items-center gap-2">
        ...
        <div x-show="activeTab === 'attendance'" x-transition>
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                        ...
                        <form id="filter-form" class="flex items-center gap-3 no-print">
                            <input type="month" id="month-input" name="month" value="{{ $selectedMonth }}" max="{{ now()->format('Y-m') }}"
                                class="bg-slate-50 border-none rounded-xl text-sm font-bold text-slate-600 px-4 py-2 focus:ring-2 focus:ring-emerald-500">
                                    <a href="{{ route('employee.printdtr', ['employee' => $employee->employee_id, 'month' => \Carbon\Carbon::parse($selectedMonth)->month, 'year' => \Carbon\Carbon::parse($selectedMonth)->year]) }}" 
                                    id="print-dtr-btn" 
                                    target="_blank" 
                ...
                <div class="overflow-x-auto" id="attendance-container">
                    @include('partials.employees._monthly_attendance_table')
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.getElementById('month-input').addEventListener('change', function() {
        ...
        let printUrl = "{{ route('employee.printdtr', ['month' => ':month', 'year' => ':year']) }}";
        printBtn.href = printUrl.replace(':month', month).replace(':year', year);
        
        const container = document.getElementById('attendance-container');
        container.style.opacity = '0.5';

        fetch(`{{ route('employee.profile') }}?month=${monthVal}`, {
